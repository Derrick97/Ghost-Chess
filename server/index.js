...
app.post('/makeMove', (req, res) => {
    ...
    let firstCell = JSON.parse(redisClient.lindex('gameState', body.firstIndex));
    let secondCell = JSON.parse(redisClient.lindex('gameState', body.secondIndex));
    redisClient.lset('gameState', body.firstIndex, JSON.stringify({ col: firstCell.col, row: firstCell.row, piece: null}));
    redisClient.lset('gameState', body.secondIndex, JSON.stringify({ col: secondCell.col, row: secondCell.row, piece: firstCell.piece}));
    redisClient.lrange('gameState', 0, -1, function (err, reply) {
        res.json(reply.map(obj => JSON.parse(obj)));
    });
});
